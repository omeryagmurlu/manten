...
def with_state_dict(*attrs):
    def decorator(cls):
        def state_dict(self):
            parent_state = super(cls, self).state_dict() if hasattr(super(cls, self), 'state_dict') else {}
...
def with_shallow_copy(*attrs):
    def decorator(cls):
        def copy(self):
            cpy = super(cls, self).copy() if hasattr(super(cls, self), 'copy') else self.__class__()
            for attr in attrs:
