...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, dim_embed, max_len=500):
        ...
        div_term = torch.exp(
            torch.arange(0, dim_embed, 2, dtype=torch.float)
...
class SelfAttention(nn.Module):
    def __init__(
        self, dim, num_heads=8, qkv_bias=True, qk_scale=None, attn_drop=0.0, proj_drop=0.0
    ):
        super().__init__()
...
class CrossAttention(nn.Module):
    ...
    def forward(self, tgt, src, src_padded_mask=None):
        ...
        kv = (
            self.kv(src)
            .reshape(B, N_src, 2, self.num_heads, self.head_dim)
            .permute(2, 0, 3, 1, 4)
        )
        k, v = kv[0], kv[1]
        B, N_tgt, C = tgt.shape
        ...
        q = (
            self.q(tgt)
            .reshape(B, N_tgt, 1, self.num_heads, self.head_dim)
            .permute(2, 0, 3, 1, 4)[0]
        )
