...
import manten.networks.vendor.diffusion_policy.common.tensor_util as tu
...
class CropRandomizer(nn.Module):
    ...
    def __init__(
        ...
        assert len(input_shape) == 3  # (C, H, W)  # noqa: PLR2004
    ...
    def output_shape_in(self, _input_shape=None):
        """
    ...
    def forward_in(self, inputs):
        ...
        """
        assert (
            len(inputs.shape) >= 3  # noqa: PLR2004
        )  # must have at least (C, H, W) dimensions
        if self.training:
...
def crop_image_from_indices(images, crop_indices, crop_height, crop_width):
    ...
    assert crop_indices.shape[-1] == 2  # noqa: PLR2004
