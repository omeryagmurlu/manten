...
class SetAbstraction(nn.Module):
    ...
    def __init__(
        ...
        **_kwargs,
        ...
        if self.use_res:
            self.skipconv = (
                create_convblock1d(in_channels, channels[-1], norm_args=None, act_args=None)
        ...
        for i in range(len(channels) - 1):
            convs.append(  # noqa: PERF401
                create_conv(
                    ...
                    act_args=None
                    if i == len(channels) - 2 and (self.use_res or is_head)
                    else act_args,
                    **conv_args,
                )
            )
...
class FeaturePropogation(nn.Module):
    ...
    def __init__(self, mlp, upsample=True, norm_args=None, act_args=None):
        """
        Args:
        ...
        if not upsample:
            ...
            for i in range(1, len(mlp) - 1):
                linear1.append(  # noqa: PERF401
                    create_convblock1d(
                        mlp[i], mlp[i + 1], norm_args=norm_args, act_args=act_args
                    )
                )
...
class PointNextEncoder(nn.Module):
    ...
    def __init__(
        ...
        aggr_args: dict | None = None,
        group_args: dict | None = None,
        sa_layers: int = 1,
        sa_use_res: bool = False,  # noqa: FBT001
        forward_routes_to: str = "forward_seg_feat",
        **kwargs,
    ):
        if group_args is None:
            group_args = {"NAME": "ballquery"}
        ...
        self.forward_routes_to = forward_routes_to

        if isinstance(block, str):
            block = eval(block)  # noqa: S307
        self.blocks = blocks
    ...
    def _to_full_list(self, param, param_scaling=1):
        ...
        if isinstance(param, list):
            ...
            for i, value in enumerate(param):
                value = [value] if not isinstance(value, list) else value  # noqa: PLW2901
                if len(value) != self.blocks[i]:
                    value += [value[-1]] * (self.blocks[i] - len(value))  # noqa: PLW2901
    ...
    def forward(self, p0, f0=None):
        if self.forward_routes_to == "forward_cls_feat":
            return self.forward_cls_feat(p0, f0)
        elif self.forward_routes_to == "forward_seg_feat":
            return self.forward_seg_feat(p0, f0)
        else:
            raise ValueError(f"Unknown forward route: {self.forward_routes_to}")
    ...
    def forward(self, p, f):
        for i in range(-1, -len(self.decoder) - 1, -1):
            f[i - 1] = self.decoder[i][1:](
                [p[i], self.decoder[i][0]([p[i - 1], f[i - 1]], [p[i], f[i]])]
            )[1]
...
class PointNextPartDecoder(nn.Module):
        ...
        **kwargs,
        ...
        if self.cls_map == "curvenet":
            ...
            self.global_conv2 = nn.Sequential(
                create_convblock1d(
                    fp_channels[-1] * 2, 128, norm_args=None, act_args=act_args
                )
            )
            self.global_conv1 = nn.Sequential(
                create_convblock1d(fp_channels[-2] * 2, 64, norm_args=None, act_args=act_args)
            )
            skip_channels[0] += 64 + 128 + 16  # shape categories labels
        elif self.cls_map == "pointnet2":
            self.convc = nn.Sequential(
                create_convblock1d(16, 64, norm_args=None, act_args=act_args)
            )
            ...
            self.convc = nn.Sequential(
                create_convblock1d(50, 64, norm_args=None, act_args=act_args)
            )
            skip_channels[0] += 64  # shape categories labels
            self.cls2partembed = cls2partembed
    ...
    def forward(self, p, f, cls_label):
        ...
        if self.cls_map == "curvenet":
            ...
            self.cls2partembed = self.cls2partembed.to(p[0].device)
            cls_one_hot = (
                self.cls2partembed[cls_label.squeeze()].unsqueeze(-1).expand(-1, -1, N)
            )
            cls_one_hot = self.convc(cls_one_hot)

        for i in range(-1, -len(self.decoder), -1):
            f[i - 1] = self.decoder[i][1:](
                [p[i - 1], self.decoder[i][0]([p[i - 1], f[i - 1]], [p[i], f[i]])]
            )[1]
        ...
        f[-len(self.decoder) - 1] = self.decoder[0][1:](
            [
                p[1],
                self.decoder[0][0]([p[1], torch.cat([cls_one_hot, f[1]], 1)], [p[2], f[2]]),
            ]
        )[1]
