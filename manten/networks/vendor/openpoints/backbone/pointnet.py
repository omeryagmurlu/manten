...
class STNkd(nn.Module):
    def __init__(self, k=64):
        ...
        self.k = k
        self.iden = torch.from_numpy(np.eye(self.k).flatten().astype(np.float32)).reshape(
            1, self.k * self.k
        )
...
class PointNetEncoder(nn.Module):
    ...
    def __init__(
        self,
        in_channels: int,
        input_transform: bool = True,  # noqa: FBT001
        feature_transform: bool = True,  # noqa: FBT001
        is_seg: bool = False,  # noqa: FBT001
        **_kwargs,
    ):
        """_summary_
    ...
    def forward(self, x, features=None):  # noqa: ARG002
