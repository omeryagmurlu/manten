...
class EdgeConvLayer(nn.Module):
    def __init__(self, in_planes, out_planes, share_planes=8, nsample=16):
        ...
        self.conv = nn.Sequential(
            nn.Conv2d(in_planes * 2, out_planes, kernel_size=1, bias=False),
...
class PointNet2EdgeConvLayer(nn.Module):
    def __init__(self, in_planes, out_planes, share_planes=8, nsample=16):
        ...
        # self.linear_q = nn.Linear(in_planes, mid_planes)
        self.conv = nn.Sequential(
            nn.Conv1d(in_planes + 3, out_planes, kernel_size=1, bias=False),
            nn.BatchNorm1d(out_planes),
            nn.ReLU(inplace=True),
        )
    ...
    def forward(self, pxo) -> torch.Tensor:
        p, x, o = pxo  # (n, 3), (n, c), (b)
        x_k = (
            pointops.queryandgroup(self.nsample, p, p, x, None, o, o, use_xyz=True)
            .transpose(1, 2)
            .contiguous()
        )
...
class PTSeg(nn.Module):
    def __init__(
        ...
        self.dec5 = self._make_dec(
            block, planes[4], 2, share_planes, nsample[4], True
        )  # transform p5
        self.dec4 = self._make_dec(
            block, planes[3], 2, share_planes, nsample[3]
        )  # fusion p5 and p4
        self.dec3 = self._make_dec(
            block, planes[2], 2, share_planes, nsample[2]
        )  # fusion p4 and p3
        self.dec2 = self._make_dec(
            block, planes[1], 2, share_planes, nsample[1]
        )  # fusion p3 and p2
        self.dec1 = self._make_dec(
            block, planes[0], 2, share_planes, nsample[0]
        )  # fusion p2 and p1
