...
def create_grouper(group_args, support_same_as_query=True):
    ...
    if method in ["ballquery", "ball", "query"]:
        if support_same_as_query:
            ...
            grouper = lambda x, b: radius_graph(
                x, radius, b, max_num_neighbors=nsample, loop=True, flow="target_to_source"
            )
        else:
            # support, query,
            grouper = lambda x, y, b_x, b_y: ballquery(
                x, y, radius, b_x, b_y, max_num_neighbors=nsample
            )
...
class SetAbstraction(nn.Module):
    ...
    def __init__(
        ...
        if self.use_res:
            self.skipconv = (
                create_linearblock(in_channels, channels[-1], norm_args=None, act_args=None)
        ...
        for i in range(len(channels) - 1):
            convs.append(
                create_conv(
                    ...
                    channels[i + 1],
                    norm_args=norm_args,
                    act_args=None if i == len(channels) - 2 and self.use_res else act_args,
                    **conv_args,
                )
            )
        self.convs = nn.Sequential(*convs)
    ...
    def forward(self, pxb):
        ...
        if self.is_head:
            ...
            dp = torch.index_select(p, 0, edge_index[1]) - torch.index_select(
                p, 0, edge_index[0]
            )
            xj = torch.index_select(x, 0, edge_index[1])
...
class FeaturePropogation(nn.Module):
    ...
    def __init__(
        self, mlp, upsample=True, norm_args={"norm": "bn1d"}, act_args={"act": "relu"}
    ):
        """
        Args:
    ...
    def forward(self, pxb1, pxb2=None):
        ...
        if pxb2 is None:
            ...
            p1, x1, b1 = pxb1
            p2, x2, b2 = pxb2
            x = self.convs(torch.cat((x1, knn_interpolate(x2, p2, p1, b2, b1, k=3)), dim=1))
        return x
        ...
        for i in range(len(channels) - 1):
            pwconv.append(
                create_linearblock(
                    ...
                    channels[i + 1],
                    norm_args=norm_args,
                    act_args=act_args if (i != len(channels) - 2) else None,
                    **conv_args,
                )
            )
    ...
    def forward(self, p, f, b):
        for i in range(-1, -len(self.decoder) - 1, -1):
            f[i - 1] = self.decoder[i][1:](
                [p[i], self.decoder[i][0]([p[i - 1], f[i - 1], b[i - 1]], [p[i], f[i], b[i]])]
            )[1]
