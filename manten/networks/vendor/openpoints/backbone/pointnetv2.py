...
from .layers import (  # grid_subsampling,
    LocalAggregation,
    create_convblock1d,
        ...
        **_kwargs,
        ...
        for i in range(len(radii)):
            ...
            self.local_aggregations.append(
                LocalAggregation(
                    channels, aggr_args, conv_args, norm_args, act_args, group_args, use_res
                )
            )

    def forward(
        self, support_xyz: torch.Tensor, support_features: torch.Tensor = None, query_xyz=None
    ):
        r"""
        ...
        if query_xyz is None and self.stride > 1:
            idx = self.sample_fn(support_xyz, support_xyz.shape[1] // self.stride).long()
        ...
        if isinstance(param, list):
            ...
            for i, value in enumerate(param):
                ...
                if len(value) != blocks[i]:
                    value += [value[-1]] * (blocks[i] - len(value))  # noqa: PLW2901
...
class PointNet2PartDecoder(nn.Module):
    ...
    def __init__(
        ...
        group_args: dict,  # noqa: ARG002
        conv_args: dict,  # noqa: ARG002
        norm_args: dict,  # noqa: ARG002
        act_args: dict,  # noqa: ARG002
        mlps=None,
        blocks: list | None = None,
        width: int | None = None,
        strides=(4, 4, 4, 4),
        ...
        fp_mlps=None,
        decoder_layers=1,
        decocder_aggr_args=None,  # noqa: ARG002
        width_scaling=2,
        radius_scaling=2,
        nsample_scaling=1,
        use_res=False,  # noqa: ARG002
        stem_conv=False,
        ...
        if isinstance(param, list):
            ...
            for i, stride in enumerate(self.strides):
                if stride == 1:
                    ...
                    param_list.append([param] + [param * param_scaling] * (blocks[i] - 1))
                    param *= param_scaling
        return param_list
    ...
    def forward(self, l_xyz, l_features, cls_label):
        ...
        l_features[0] = self.FP_modules[0](
            l_xyz[0], l_xyz[1], torch.cat([cls_one_hot, l_features[0]], 1), l_features[1]
        )
        return l_features[0]
