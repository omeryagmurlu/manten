...
class ASSA(nn.Module):
    def __init__(
        ...
        mid_conv_in_channel = CHANNEL_MAP[self.feature_type](channels[num_preconv])
        ...
        for i in range(num_preconv, len(channels) - 1):
            convs.append(
                create_convblock1d(
                    ...
                    channels[i + 1],
                    norm_args=norm_args,
                    act_args=None if use_res and i == len(channels) - 2 else act_args,
                    **conv_args,
                )
            )
        ...
        if use_res:
            self.skip_layer = (
                nn.Identity()
                if skip_channels == channels[-1]
                else nn.Conv1d(skip_channels, channels[-1], 1, bias=False)
            )
    ...
    def forward(self, query_xyz, support_xyz, features, query_idx=None):
        ...
        fj = fj.unsqueeze(1).expand(-1, 3, -1, -1, -1) * dp.unsqueeze(2)
...
class ConvPool(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        if use_res:
            self.skipconv = (
                create_convblock1d(
                    skip_channel, channels[-1], norm_args=None, act_args=None, **conv_args
                )
                if skip_channel != channels[-1]
                else nn.Identity()
            )
    ...
    def forward(self, query_xyz, support_xyz, features, query_idx=None):
        ...
        # """End of debug"""
        fj = get_aggregation_feautres(
            query_xyz, dp, features, fj, feature_type=self.feature_type
        )
...
class LocalAggregation(nn.Module):
    def __init__(
        ...
        if aggr_type.lower() == "convpool":
            self.SA_CONFIG_operator = ConvPool(
                channels,
                conv_args,
                norm_args,
                act_args,
                ...
                reduction,
                use_res,
                use_pooled_as_identity,
            )
        elif aggr_type.lower() == "assa":
            self.SA_CONFIG_operator = ASSA(
                channels,
                conv_args,
                norm_args,
                act_args,
