...
from .group import KNNGroup, QueryAndGroup, get_aggregation_feautres
...
class SubsampleGroup(nn.Module):
    ...
    def forward(self, p, x=None):
        if (
            "fps" in self.subsample.lower()
            or "furthest" in self.subsample.lower()
            or "farthest" in self.subsample.lower()
        ):
            ...
            raise NotImplementedError(
                f"{self.subsample.lower()} is not implemented. Only support fps, random"
            )
        center_p = torch.gather(
            p, 1, idx.unsqueeze(-1).expand(-1, -1, 3)
        )  # downsampled point cloud, [B, npoint, 3]
        if x is not None:
            B, C, N = x.shape[:3]
...
class PointPatchEmbed(nn.Module):
    ...
    def __init__(  # noqa: C901, PLR0912
        ...
        **_kwargs,
        ...
        if "ball" in self.group or "query" in self.group:
            ...
            self.grouper = KNNGroup(
                self.group_size, relative_xyz=relative_xyz, normalize_dp=normalize_dp
            )
        else:
            raise NotImplementedError(
                f"{self.group.lower()} is not implemented. Only support ballquery, knn"
        ...
        if channels is None:
            channels = (
                [CHANNEL_MAP[feature_type](in_channels)]
                + [embed_dim] * (layers // 2)
    ...
    def forward(self, p, x=None):
        ...
        fj = torch.cat([self.pool(fj).expand(-1, -1, -1, self.group_size), fj], dim=1)
