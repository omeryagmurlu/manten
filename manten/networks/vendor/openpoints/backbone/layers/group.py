...
class KNN(nn.Module):
    def __init__(self, neighbors, transpose_mode=True):  # noqa: ARG002
        super().__init__()
...
class BallQuery(Function):
    ...
    @staticmethod
    def backward(ctx, a=None):  # noqa: ARG004
...
class QueryAndGroup(nn.Module):
    def __init__(
        ...
        **_kwargs,
        ...
        assert (
            self.normalize_dp + self.normalize_by_std + self.normalize_by_allstd < 2  # noqa: PLR2004
...
class GroupAll(nn.Module):
    ...
    def forward(
        self,
        new_xyz: torch.Tensor,  # noqa: ARG002
        xyz: torch.Tensor,
        features: torch.Tensor = None,
    ):
        """
...
def get_aggregation_feautres(p, dp, f, fj, feature_type="dp_fj"):
    if feature_type == "dp_fj":
        ...
        df = fj - f.unsqueeze(-1)  # noqa: PD901
        fj = torch.cat([dp, fj, df], 1)
    elif feature_type == "pi_dp_fj_df":
        df = fj - f.unsqueeze(-1)  # noqa: PD901
        fj = torch.cat(
            [p.transpose(1, 2).unsqueeze(-1).expand(-1, -1, -1, df.shape[-1]), dp, fj, df], 1
        )
...
if __name__ == "__main__":
    ...
    # --------------- debug downsampling
    from openpoints.models.layers.layer3d import (  # type: ignore  # noqa: PGH003
