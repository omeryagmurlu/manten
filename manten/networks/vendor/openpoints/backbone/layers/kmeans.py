...
class KMeansEmbed(nn.Module):
    ...
    def __init__(
        self, in_chans=3, num_groups=256, encoder_dim=256, feature_type="dp", **kwargs
    ):
        super().__init__()
        ...
        self.kmeans = MultiKMeans(
            n_clusters=num_groups, n_kmeans=32, mode="euclidean", verbose=0
        )
...
if __name__ == "__main__":
    ...
    from openpoints.dataset import ModelNet, vis_points
    from openpoints.models.layers import fps

    B, C, N = 8, 3, 8196
    device = "cuda"

    dataset = ModelNet("data/ModelNet40", N, 40, split="test")
