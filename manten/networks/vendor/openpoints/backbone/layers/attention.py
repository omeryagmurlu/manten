...
from . import DropPath, Mlp, create_norm
...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        q, k, v = qkv.unbind(
            0
        )  # make torchscript happy (cannot use tensor as tuple), shape [B, #Heads, N, C]
...
class Block(nn.Module):
    def __init__(
        ...
        self.mlp = Mlp(
            in_features=dim, hidden_features=mlp_hidden_dim, act_args=act_args, drop=drop
        )
...
class TransformerEncoder(nn.Module):
    ...
    def forward_features(self, x, pos, num_outs=None):
        ...
        out_depth = list(range(self.depth))[
            (self.depth - (num_outs - 1) * dilation - 1) :: dilation
        ]
