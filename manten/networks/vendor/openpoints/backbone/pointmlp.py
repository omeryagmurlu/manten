...
def get_activation(activation):  # noqa: PLR0911
...
class LocalGrouper(nn.Module):
    def __init__(
        self, channel, sample_ratio, kneighbors, use_xyz=True, normalize="center", **_kwargs
    ):
        """
    ...
    def forward(self, xyz, points):
        ...
        if self.normalize is not None:
            ...
            if self.normalize == "anchor":
                mean = (
                    torch.cat([new_points, new_xyz], dim=-1) if self.use_xyz else new_points
                )
...
class PosExtraction(nn.Module):
    def __init__(
        self, channels, blocks=1, groups=1, res_expansion=1, bias=True, activation="relu"
    ):
        """
        ...
        **_kwargs,
        ...
        for i in range(len(pre_blocks)):
            ...
            local_grouper = LocalGrouper(
                last_channel, reduce, kneighbor, use_xyz, normalize
            )  # [b,g,k,d]
...
def pointMLP(num_classes=40, **kwargs) -> PointMLPEncoder:  # noqa: N802
...
def pointMLPElite(num_classes=40, **kwargs) -> PointMLPEncoder:  # noqa: N802
    return PointMLPEncoder(
        num_classes=num_classes,
        embed_dim=32,
        groups=1,
