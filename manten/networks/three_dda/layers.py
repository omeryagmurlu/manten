...
class RelativeCrossAttentionLayer(nn.Module):
    ...
    def forward(
        self, query, value, diff_ts=None, query_pos=None, value_pos=None, pad_mask=None
    ):
        if diff_ts is not None:
...
class FFWRelativeCrossAttentionModule(nn.Module):
    def __init__(self, embedding_dim, num_attn_heads, num_layers, use_adaln=True):
        super().__init__()
    ...
    def forward(self, query, value, diff_ts=None, query_pos=None, value_pos=None):
        output = []
        for i in range(self.num_layers):
            query = self.attn_layers[i](query, value, diff_ts, query_pos, value_pos)
...
class FFWRelativeSelfCrossAttentionModule(nn.Module):
    def __init__(
        ...
        cross_inds = np.linspace(
            0, num_self_attn_layers, num_cross_attn_layers + 1, dtype=np.int32
        ).tolist()
    ...
    def forward(self, query, context, diff_ts=None, query_pos=None, context_pos=None):
        output = []
        for i in range(self.num_layers):
