# ruff: noqa
# https://github.com/HaoyiZhu/PointCloudMatters/blob/main/src/data/components/transformpcd.py
...
class CollectPCD:
    def __init__(self, keys, offset_keys_dict=dict(offset="coord"), stack_keys=(), **kwargs):
        """
...
class ChromaticAutoContrastPCD:
...
class RandomColorGrayScalePCD:
    ...
    def rgb_to_grayscale(color, num_output_channels=1):
        if color.shape[-1] < 3:
            raise TypeError(
                f"Input color should have at least 3 dimensions, but found {color.shape[-1]}"
            )
...
class HueSaturationTranslationPCD:
    @staticmethod
    def rgb_to_hsv(rgb):
...
class ComposePCD:
    ...
    def __call__(self, data_dict, mode="train"):
        for i, t in enumerate(self.transforms):
            if mode != "train" and (
                "rand" in t.__class__.__name__.lower()
                or "jitter" in t.__class__.__name__.lower()
                or "shuffle" in t.__class__.__name__.lower()
            ):
                continue
