...
for rigid_task in {"PegInsertionSide-v1","PickCube-v1"}
do
...
for rigid_task in {"PegInsertionSide-v1","PickCube-v1"} # PickCube-v1, PegInsertionSide-v1
do
for obs_mode in {"state","pointcloud"} # rgbd, state, pointcloud
do
python -m mani_skill.trajectory.replay_trajectory \
  --traj-path "demos/$rigid_task/motionplanning/trajectory.h5" \
  --save-traj --target-control-mode pd_ee_delta_pose --obs-mode "$obs_mode" --num-envs 15
  # --count 30
  # --use-first-env-state \
done
done
...
#   --save-traj --target-control-mode pd_ee_delta_pose --obs-mode "$obs_mode" --num-envs 10 -b cpu \
#   # --use-first-env-state \
#   --count 150
# done
