...
def get_gripper_loc_bounds(path: str, buffer: float = 0.0, task: Optional[str] = None):
    gripper_loc_bounds = json.load(open(path))
    if task is not None and task in gripper_loc_bounds:
        ...
        gripper_loc_bounds_min = (
            np.min(np.stack([bounds[0] for bounds in gripper_loc_bounds.values()]), axis=0)
            - buffer
        )
        gripper_loc_bounds_max = (
            np.max(np.stack([bounds[1] for bounds in gripper_loc_bounds.values()]), axis=0)
            + buffer
        )
        gripper_loc_bounds = np.stack([gripper_loc_bounds_min, gripper_loc_bounds_max])
