...
############################################################
def prepare_visual_states(obs: Dict[str, Dict[str, Any]], env: PlayTableSimEnv):
...
def prepare_proprio_states(obs: Dict[str, Dict[str, Any]], env: PlayTableSimEnv):
...
def convert_action(trajectory):
    ...
    if (_rot != _rot).any():
        ...
        rotation = _rot
    openness = (2 * (openness >= 0.5).long() - 1).data.cpu().numpy()
