...
class RolloutVideo:
    ...
    def draw_outcome(self, successful):
        """
        Draw red or green border around video depending on successful execution
    ...
    def update(self, rgb_obs: torch.Tensor) -> None:
        ...
        """
        rgb_obs = ((rgb_obs.float() - 127) / 255) * 2
    ...
    def add_language_instruction(self, instruction):
        ...
        self.videos[-1][:, self.sub_task_beginning :, ...] += torch.from_numpy(
            img_text.round()
        )
    ...
    def _log_videos_to_wandb(self):
        ...
        for (task, path), caption in zip(video_paths.items(), captions):
            self.logger.experiment.log(
                {f"video{task}": wandb.Video(path, fps=10, format="gif", caption=caption)}
            )
